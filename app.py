...
<script>
    ...
    function startProcess() {
        ...
        .then(data => {
            ...
            } else { alert("Error: Engine busy. Try refreshing the page or use a different link!"); }
        }).catch(err => {
            document.getElementById("fullLoader").style.display = "none";
            document.getElementById("mainBtn").style.display = "block";
...
def download():
    ...
    # Naya robust scraper engine
    headers = {
        "X-RapidAPI-Key": RAPID_API_KEY,
        "X-RapidAPI-Host": "all-video-downloader-all-in-one.p.rapidapi.com"
    }
    try:
        # Step 1: Link Clean
        clean_url = url.split('?')[0] if "instagram.com" in url else url
        
        # Step 2: Fetch from a more stable API for Replit/Vercel
        response = requests.get(
            "https://all-video-downloader-all-in-one.p.rapidapi.com/download",
        ...
        if links:
            # Sabse high quality link dhundhna
            best_link = links[0].get('link')
            thumbnail = response.get('picture', '')
