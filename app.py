...
<head>
    ...
    <style>
        * { box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        .main-card { max-width: 440px; width: 92%; padding: 40px 25px; border-radius: 25px; background: rgba(15, 10, 25, 0.7); backdrop-filter: blur(25px); border-top: 2px solid rgba(254, 9, 121, 0.5); border-bottom: 2px solid rgba(0, 242, 254, 0.5); box-shadow: 0 20px 50px rgba(0, 0, 0, 0.8); margin-top: 20px; position: relative; }
...
<script>
    ...
    function startProcess() {
        ...
        .then(data => {
            ...
            } else { alert("Server Error! Link not found."); }
        }).catch(() => {
            document.getElementById("fullLoader").style.display = "none";
            document.getElementById("mainBtn").style.display = "block";
...
def download():
    ...
    try:
        res = requests.post("https://social-download-all-in-one.p.rapidapi.com/v1/social/autolink", json={"url": url}, headers=headers, timeout=8).json()
        media_url = res.get('hd') or res.get('video') or res.get('url')
        m_type = "video"
        for m in res.get('medias', []):
            if m.get('type') == 'image' and not media_url: 
                media_url = m.get('url'); m_type = "image"
