...
<div class="seo-section">
    ...
    <div class="faq-item">
        ...
        <p>Save Pro is the ultimate <b>Instagram reel downloader</b>. Just paste the reel link, hit unlock, and save the HD MP4 file directly to your gallery.</p>
    </div>
</div>
...
<script>
    ...
    async function pasteFromClipboard() { try { const text = await navigator.clipboard.readText(); document.getElementById("videoUrl").value = text; } catch (err) {} }
    ...
    function startProcess() {
        ...
        .then(data => {
            ...
            if(data.success) {
                ...
                showToast("✅ Ready!");
            } else { alert("❌ Media Not Found! Try another link."); }
        }).catch(() => { 
...
def download():
    ...
    # Backend Engine Fix (Strictly using the most stable API endpoint)
    ...
    try:
        ...
        media_url = None; thumbnail = res.get('thumbnail') or res.get('image') or ""
        
        # Priority check for Video
        for m in res.get('medias', []):
            if m.get('type') == 'video' or 'mp4' in str(m.get('url')):
                media_url = m.get('url'); break
        
        # Fallback to direct links if list is empty
        if not media_url:
            media_url = res.get('hd') or res.get('video') or res.get('url')
            
        if media_url:
            return jsonify({"success": True, "media_url": media_url, "media_type": "video", "thumbnail": thumbnail, "title": res.get('title')})
